...
noise_factor = 0.5
train_X_noisy = train_X + noise_factor * numpy.random.normal(loc=0.0, scale=1.0, size=train_X.shape) 
public_test_X_noisy = public_test_X + noise_factor * numpy.random.normal(loc=0.0, scale=1.0, size=public_test_X.shape) 

train_X_noisy = numpy.clip(train_X_noisy, 0., 1.)
public_test_X_noisy = numpy.clip(public_test_X_noisy, 0., 1.)
...
start = time.time()
#Fit the model
model.fit_generator(datagen.flow(train_X_noisy, train_Y, batch_size=256), epochs=10, validation_data = (public_test_X_noisy, public_test_Y))
stop = time.time()
